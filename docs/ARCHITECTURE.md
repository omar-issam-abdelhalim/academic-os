...
| Hosting | Netlify (recommended) | Vercel, Cloudflare Pages, GitHub Pages — any static host works since this is a backend-less SPA. Netlify is recommended for: zero-config GitHub integration, automatic PR deploy previews, atomic deploys with one-click rollback, generous free tier, easy custom domain. Cloudflare Pages and Vercel are equally valid; **final choice is an open question** (see STAGE_0_REPORT.md) in case the user already has a preferred account/ecosystem. |
...
- **`index.html` / navigation requests**: served with short/no-cache semantics (host default headers are adequate on Netlify/Vercel/Cloudflare Pages) so clients reliably discover new deployments instead of being pinned to a stale shell indefinitely.
...
See STAGE_0_REPORT.md §"Open Questions" for the consolidated list requiring product-owner input (hosting provider choice, license, tag scoping, GitHub repo visibility).
