...
| Charting | Recharts — **installed and consumed starting Stage 4** (the redefined analytics stage; originally slotted for "Stage 6" under the pre-Stage-3 numbering, before the product owner's Stage 3/4 task prompts collapsed the later stages forward) | visx, Chart.js, D3 directly — Recharts is React-idiomatic, SVG-based (accessible, stylable), and sufficient for the trend/comparison charts described in the product spec; D3-direct or visx are lower-level than needed. `npm audit` confirms it introduces zero new advisories (the pre-existing 14 are all in the unrelated eslint/react-router/workbox dev-dependency chains, documented since Stage 2). |
...
Dependency count is intentionally small: React, React Router, Dexie, dexie-react-hooks, Zod, Recharts (Stage 4), and JSZip (Stage 5, used by Media Export) as runtime dependencies; Vite, vite-plugin-pwa, TypeScript, Vitest, Testing Library, fake-indexeddb, ESLint, Prettier as dev dependencies. No CSS framework is mandated by Stage 0 — that is a Stage 1 design-system decision.
...
Reliable "notify me before class even if the app/browser is fully closed" requires either (a) a push-capable backend server holding subscriptions and sending Push API messages, or (b) the browser proactively waking the app via Periodic Background Sync — both are inconsistently supported (especially on iOS) and (a) directly conflicts with the local-first/no-mandatory-backend requirement. The plan below was honest and documented from Stage 0; Tiers 1 and 2 are now real:

1. **Baseline — implemented (Stage 5):** `src/domain/notifications.ts` (pure "what's starting soon" planning, a 10-minute default lead time) + `src/features/shared/useClassReminders.ts` (the reactive hook, mounted once at the app-shell level via `src/app/ClassReminderBanner.tsx`) + `src/app/AppShell.tsx`. Rechecks on mount, on window `focus`, and on a 30-second poll (so a tab left open and idle still surfaces a reminder) — works everywhere, no platform dependency. Gated behind the existing `AppPreferences.notificationsEnabled` "Class reminders" Settings toggle.
2. **Best-effort enhancement — implemented (Stage 5):** the same hook schedules a real `Notification` via `setTimeout` for every still-future class today, but only if the browser's Notification permission is already `"granted"` — requested explicitly in `SettingsScreen.tsx` only in direct response to the user turning the toggle on (never auto-prompted on load, so it can never "nag" with a repeat prompt: a browser that already denied it just resolves `"denied"` again with no UI). Degrades silently to nothing if the API is unavailable, permission isn't granted, or the tab isn't open when the timer would have fired — documented to the user in Settings as "reminders work while Academic OS is open or recently used," not as a guaranteed background alarm, and (if permission is denied) with one calm, non-repeating inline hint explaining the limitation — never a nagging re-prompt.
